...
export default function PythonPage() {
  ...
  const weeks = [1, 2, 3].map(weekNum => {
    const weekChallenges = allChallenges.filter(c => {
      const dayNum = c.dayNumber;
      return dayNum >= (weekNum - 1) * 7 + 1 && dayNum <= weekNum * 7;
    });

    // Group by day within the week
    const days = Array.from({ length: weekNum === 3 ? 1 : 7 }, (_, i) => i + 1).map(day => ({
      day,
      challenges: weekChallenges.filter(c => {
        const dayInWeek = c.dayNumber - (weekNum - 1) * 7;
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-12 lg:py-16">
          <div className="max-w-2xl">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <div className="px-2.5 sm:px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                <span className="text-xs sm:text-sm font-semibold text-emerald-400">Complete Python Curriculum</span>
              </div>
              <div className="px-2.5 sm:px-3 py-1 bg-zinc-800 border border-zinc-700 rounded-full">
                <span className="text-xs sm:text-sm font-medium text-zinc-300">3 Weeks</span>
              </div>
            </div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-zinc-100 mb-3 sm:mb-4 tracking-tight">
              Learn Python in <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-blue-400">3 Weeks</span>
            </h1>
            <p className="text-base sm:text-lg lg:text-xl text-zinc-400 mb-6 sm:mb-8 leading-relaxed">
              {allChallenges.length} interactive challenges taking you from variables and loops to object-oriented programming and a portfolio project.
            </p>
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 sm:gap-6 text-xs sm:text-sm text-zinc-500">
              <div className="flex items-center gap-1.5 sm:gap-2">
                ...
                <span>{allChallenges.length} Challenges</span>
              </div>
              <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {week.days.map((dayInfo) => {
                ...
                return (
                  <Link
                    key={dayInfo.day}
                    href={firstChallenge ? `/python-challenge/${firstChallenge.slug}` : '#'}
                    className="group"
                  >
                    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 sm:p-5 hover:border-zinc-700 hover:bg-zinc-900/50 transition-all h-full">
                      ...
                      <h3 className="text-sm sm:text-base font-semibold text-zinc-100 group-hover:text-emerald-400 transition-colors mb-1.5 sm:mb-2 line-clamp-2">
                        {dayInfo.title}
                      </h3>
...
function getWeekTitle(week: number): string {
  const titles: Record<number, string> = {
    ...
    2: 'Data Structures & OOP Essentials',
    3: 'Portfolio Project',
  };
...
function getWeekDescription(week: number): string {
  const descriptions: Record<number, string> = {
    1: 'Master variables, conditionals, loops, functions, and basic data structures',
    2: 'Deep dive into advanced data structures and object-oriented programming',
    3: 'Build a complete, resume-worthy project applying all learned concepts',
  };
...
function getDayTitle(week: number, day: number): string {
  const titles: Record<string, string> = {
    ...
    // Week 2
    '2-1': 'Advanced Lists',
    '2-2': 'Advanced Dictionaries',
    '2-3': 'Tuples & Sets',
    '2-4': 'Classes & OOP',
    '2-5': 'Methods & Properties',
    '2-6': 'OOP Practice',
    '2-7': 'Week 2 Review',

    // Week 3
    '3-1': 'Arithmetic Formatter Project',
  };
